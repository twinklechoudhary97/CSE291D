...
static int run_server()
{
	...
	for (;;) {
		...
		//  printf("Buffer value received\n"); 
		 ...
		 buf = rx_bufs[i];
	     new_buf = rte_pktmbuf_clone(buf, tx_mbuf_pool);
		//  rte_pktmbuf_dump(stdout, new_buf, 64);

		struct rte_ether_hdr *ptr_mac_hdr;

		ptr_mac_hdr = rte_pktmbuf_mtod(buf, struct rte_ether_hdr *);

		struct rte_ether_addr temp = ptr_mac_hdr->dst_addr;

		rte_ether_addr_copy(&ptr_mac_hdr->src_addr, &ptr_mac_hdr->dst_addr);
		rte_ether_addr_copy(&temp, &ptr_mac_hdr->src_addr);

		rx_ipv4_hdr = rte_pktmbuf_mtod_offset(new_buf, struct rte_ipv4_hdr *,RTE_ETHER_HDR_LEN);
		rte_be32_t temp_ipv4 = rx_ipv4_hdr->dst_addr;

		
		rx_ipv4_hdr->dst_addr = rx_ipv4_hdr->src_addr;
		rx_ipv4_hdr->src_addr = temp_ipv4;



		// struct in_addr ip_addr;
    	// ip_addr.s_addr = rx_ipv4_hdr->dst_addr;
        // const char* dst_ip=inet_ntoa(ip_addr);
		// printf("Result of Dest IP\n");
		// printf("%s\n", dst_ip);

		// ip_addr.s_addr = rx_ipv4_hdr->src_addr;
		...
		rte_udp_hdr = rte_pktmbuf_mtod_offset(new_buf, struct rte_udp_hdr *,
						RTE_ETHER_HDR_LEN + sizeof(struct rte_ipv4_hdr));
		...
		/* send packet */
		nb_tx = rte_eth_tx_burst(port, 0, &new_buf, 1);
