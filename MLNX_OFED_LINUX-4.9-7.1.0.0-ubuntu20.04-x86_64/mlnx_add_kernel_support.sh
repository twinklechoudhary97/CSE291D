...
do
	case "$1" in
		...
		exit 0
		;;
		--without-* | --with-cx4lx-optimizations)
...
if [ "X$distro" == "X" -a $deb -eq 0 ]; then
	...
	case "$distro_rpm" in
		...
		distro=rhel7.6
		if echo "$kernel" | grep -q '^4.14.*el7'; then
			distro=rhel7.6alternate
		fi
		;;
		redhat-release*-7.7*|centos-release-7-7*|sl-release-7.7*)
		distro=rhel7.7
		;;
		redhat-release*-7.8*|centos-release-7-8*|sl-release-7.8*)
		distro=rhel7.8
		;;
		redhat-release*-7.9*|centos-release-7-9*|sl-release-7.9*)
		distro=rhel7.9
		;;
		redhat-release-[4-8].[0-9]-* | \
		centos-release-[4-8].[0-9]-* | \
		centos-linux-release-8.[3-9]-* | \
		sl-release-[4-7].[0-9]-* | \
		rocky-release-8.[0-9]-* | \
		__nosuchdistro__)
		# redhat-release-8.0-0.39 => rhel8.0
		distro=$(echo $distro_rpm | sed -r -e 's/(redhat|centos|centos-linux|sl|rocky)-release-([0-9])\.([0-9]+).*/rhel\2.\3/')
		;;
		...
		distro=ol6.10
		;;
		oraclelinux-release-7.[0-9]-* | \
		oraclelinux-release-8.[0-3]-* | \
		__nosuchdistro__)
		distro=$(echo $distro_rpm | sed -r -e 's/oraclelinux-release-([0-9])\.([0-9]+).*/ol\1.\2/')
		;;
		bclinux-release-7-[3-7].*)
		distro_ver=`echo $distro_rpm | cut -d. -f1 | cut -d- -f 3-4 | tr - .`
		distro="bclinux$distro_ver"
		;;
		bclinux-release-8-1.*)
		distro_ver=`echo $distro_rpm | cut -d- -f3`
		distro="bclinux$distro_ver"
		;;
		sles-release-11-72.*)
		distro=sles11
		...
		__nosuchdistro__)
		distro_ver=`echo "$distro_rpm" | cut -d- -f3`
		distro_ver_sles=`echo "$distro_ver" | sed -e 's/\./sp/'` # 12sp4
		distro="sles$distro_ver_sles"
		;;
		...
		distro=sles15sp0
		;;
		fedora-release-1[4-9] | fedora-core-2[0-9] | feroda-core-3[0])
		...
		;;
		euleros-release-2.0SP[23589]*)
		sp=`echo ${distro_rpm} | sed -e 's/euleros-release-2\.0SP\(.\).*/\1/'`
		distro=euleros2.0sp${sp}
		;;
		alios-release-server-7.2*)
		ver=`echo ${distro_rpm} | sed -E -e 's/alios-release-server-([0-9]\.[0.9]).*/\1/'`
		distro=alios${ver}
		;;
		base-files-3.0*)
...
if [ $deb -eq 0 ]; then
	...
	if ( /bin/ls $mnt_point/RPMS/COMMON/mlnx-ofa_kernel-modules-*${rpm_kernel}[_.]*${build_arch}.rpm > /dev/null 2>&1 ) &&
		( /bin/ls $mnt_point/RPMS/COMMON/kernel-mft-*${rpm_kernel}.${build_arch}.rpm > /dev/null 2>&1 ) &&
		( /bin/ls $mnt_point/RPMS/COMMON/srp-*${rpm_kernel}.${build_arch}.rpm > /dev/null 2>&1 ) &&
		( /bin/ls $mnt_point/RPMS/COMMON/iser-*${rpm_kernel}.${build_arch}.rpm > /dev/null 2>&1 ) &&
		( /bin/ls $mnt_point/RPMS/COMMON/knem-*${rpm_kernel}.${build_arch}.rpm > /dev/null 2>&1 ) ; then
	...
	# debian
	rpm_kernel=${kernel//_/-}
	if ( /bin/ls $mnt_point/DEBS/COMMON/mlnx-ofed-kernel-modules*${rpm_kernel}_*.deb > /dev/null 2>&1 ) &&
		( /bin/ls $mnt_point/DEBS/COMMON/kernel-mft-modules*${rpm_kernel}_*.deb > /dev/null 2>&1 ) &&
		( /bin/ls $mnt_point/DEBS/COMMON/srp-modules*${rpm_kernel}_*.deb > /dev/null 2>&1 ) &&
		( /bin/ls $mnt_point/DEBS/COMMON/knem-modules*${rpm_kernel}_*.deb > /dev/null 2>&1 ) &&
		( /bin/ls $mnt_point/DEBS/COMMON/iser-modules*${rpm_kernel}_*.deb > /dev/null 2>&1 ) ; then
		pass_echo "Required kernel ($kernel) is already supported by $PACKAGE"
		cleanup
		exit 30
	fi
...
if [ "X`find $tmpdir/${iso_dir}/$BINDIR \( -name '*kmp*' -o -name '*kmod*' \) 2>/dev/null`" != "X" ]; then
	if [ "$TS" != '' ]; then
...
do
	...
	ex install -m 0644 $p $tmpdir/$iso_dir/$BINDIR/COMMON
done

# Rebuild --upstream-libs repo dir
UPSTREAM_LIBS_DIR="UPSTREAM_LIBS"
UPSTREAM_LIBS_REPO="${BINDIR}/${UPSTREAM_LIBS_DIR}"
if [ -e "$tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO" ]; then
	cd $tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO
	# kernel binaries
	bins=`find ../COMMON -name "*rpm" -o -name "*deb"`
	for p in $bins
	do
		case "$p" in
...
# Rebuild --mlnx-libs repo dir
MLNX_LIBS_DIR="MLNX_LIBS"
MLNX_LIBS_REPO="${BINDIR}/${MLNX_LIBS_DIR}"
if [ -e "$tmpdir/$iso_dir/$MLNX_LIBS_REPO" ]; then
	cd $tmpdir/$iso_dir/$MLNX_LIBS_REPO
	bins=`find ../COMMON -name "*rpm" -o -name "*deb"`
	for p in $bins
	do
		case "$p" in
...
if [ $WITH_ETH_ONLY -eq 1 ]; then
	...
	bins=`find ../${MLNX_LIBS_REPO} -name "*mlnx-en*" -o -name "*mlnx_en*" \
...
if [ $skip_repo -eq 0 ]; then
	...
	KER_META_FLAG=
	if [ "X$KMP" != "X" ] || (/bin/ls $tmpdir/$iso_dir/RPMS/COMMON/mlnx-ofa_kernel-modules-*${rpm_kernel}[_.]*${build_arch}.rpm > /dev/null 2>&1); then
		KER_META_FLAG="--kernel $kernel"
	fi
	group_list="all hpc basic vma vma-vpi vma-eth guest hypervisor kernel-only dpdk bluefield"
	METAGROUPS=
	if [ $WITH_ETH_ONLY -eq 1 ]; then
	...
	do
		$tmpdir/$iso_dir/$INSTALLER --${group} --mlnx-libs -p $DKMSFLAG -k $kernel --tmpdir $tmpdir_logs $disabled_pkgs $distro1 --skip-distro-check 2>/dev/null > $tmpdir/$iso_dir/$BINDIR/${MLNX_LIBS_DIR}/${group}_packages.txt
		$tmpdir/$iso_dir/$INSTALLER --${group} --upstream-libs -p $DKMSFLAG -k $kernel --tmpdir $tmpdir_logs $disabled_pkgs $distro1 --skip-distro-check 2>/dev/null > $tmpdir/$iso_dir/$BINDIR/${UPSTREAM_LIBS_DIR}/${group}_packages.txt
		# user space only packages list
		if [ "X${group}" != "Xkernel-only" ]; then
			$tmpdir/$iso_dir/$INSTALLER --${group} --mlnx-libs -p $DKMSFLAG -k $kernel --tmpdir $tmpdir_logs $disabled_pkgs $distro1 --skip-distro-check --user-space-only 2>/dev/null > $tmpdir/$iso_dir/$BINDIR/${MLNX_LIBS_DIR}/${group}-user-only_packages.txt
			$tmpdir/$iso_dir/$INSTALLER --${group} --upstream-libs -p $DKMSFLAG -k $kernel --tmpdir $tmpdir_logs $disabled_pkgs $distro1 --skip-distro-check --user-space-only 2>/dev/null > $tmpdir/$iso_dir/$BINDIR/${UPSTREAM_LIBS_DIR}/${group}-user-only_packages.txt
		fi
	done
	# create metadata rpms
	ex /usr/bin/perl $tmpdir/$iso_dir/create_mlnx_ofed_installers.pl --with-hpc --tmpdir $tmpdir_logs --mofed $tmpdir/$iso_dir --rpms-tdir $tmpdir/$iso_dir/$BINDIR/MLNX_LIBS --output $tmpdir/$iso_dir $DEBFLAG $KER_META_FLAG $META_BUILD_NUM $METAGROUPS
	rm -rf $tmpdir/$iso_dir/$BINDIR/MLNX_LIBS/*txt >/dev/null 2>&1
	ex /usr/bin/perl $tmpdir/$iso_dir/create_mlnx_ofed_installers.pl --with-hpc --tmpdir $tmpdir_logs --mofed $tmpdir/$iso_dir --rpms-tdir $tmpdir/$iso_dir/$BINDIR/UPSTREAM_LIBS --output $tmpdir/$iso_dir $DEBFLAG $KER_META_FLAG $META_BUILD_NUM $METAGROUPS
	rm -rf $tmpdir/$iso_dir/$BINDIR/UPSTREAM_LIBS/*txt >/dev/null 2>&1
	# update eth ETH only RPMS dir
	if [ $WITH_ETH_ONLY -eq 1 ]; then
		/bin/mv -f $tmpdir/$iso_dir/${BINDIR}/MLNX_LIBS/*eth-only* $tmpdir/$iso_dir/${BINDIR}_ETH/ || true
	fi
	# create repo
	cd $tmpdir/$iso_dir
	...
	if [ $deb -eq 0 ]; then
		...
		ex createrepo -q $GROUPFILE $tmpdir/$iso_dir/$MLNX_LIBS_REPO
